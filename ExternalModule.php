...
class ExternalModule extends AbstractExternalModule
{
     ...
     public function addressValidation($record, $event_id)
   {
       ...
       $big_bbox = [3000000, 3000000, -3000000, -3000000];

       foreach ($bbox_results as $b) {
           if($big_bbox[0]>$b->bbox[0]){
               $big_bbox[0]=$b->bbox[0];
           }
           if($big_bbox[1]>$b->bbox[1]){
               $big_bbox[1]=$b->bbox[1];
           }
           if($big_bbox[2]<$b->bbox[2]){
               $big_bbox[2]=$b->bbox[2];
           }
           if($big_bbox[3]<$b->bbox[3]){
               $big_bbox[3]=$b->bbox[3];
           }
       }

       $bbox = implode(",", $big_bbox);
       ...
       elseif ($city =='Prilly,Jouxtens-Mézery') {
                $city = 'Prilly';
       }
